"""
TpBS Backtest Runner

Test the TpBS quote model with real trade data from S3/local parquet files.
"""
...
@dataclass
class TpBSBacktestConfig:
    """Configuration for TpBS backtest."""
...
def run_tpbs_backtest(config: TpBSBacktestConfig) -> TradesBacktestResult:
    """Run backtest with TpBS quote model using real trade data."""
    ...
    if config.market_slug:
        ...
        if market_params:
            ...
            # For binary options, we use the actual strike price
            ...
            # For now, use a fixed estimate - in production, calculate dynamically
    ...
    # Create TpBS quote model
    tpbs_config = TpBSQuoteConfig(
        max_z=config.max_z,
        min_z=config.min_z,
    ...
    quote_model = TpBSQuoteModel(config=tpbs_config)
...
def print_results(result: TradesBacktestResult, config: TpBSBacktestConfig):
    """Print backtest results."""
    metrics = result.metrics

    print("\n" + "=" * 70)
    print("TpBS BACKTEST RESULTS")
    print("=" * 70)
    ...
    print(f"  TpBS max_z/min_z: {config.max_z}/{config.min_z}")
...
def main():
    """Run TpBS backtest with real data."""

    print("Running TpBS Backtest with real trade data...")

    # Example market_slug - update this based on actual data
    # Format: {asset}-{direction}-{strike}-{month}-{day}-{hour}{ampm}
    # e.g., "btc-above-100000-jan-20-1pm"
    market_slug = None  # Set to actual market slug if known

    config = TpBSBacktestConfig(
    ...
    result = run_tpbs_backtest(config)
